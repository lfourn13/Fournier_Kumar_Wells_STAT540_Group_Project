...
#example of a gene that has a "0" expression value across samples
 ID3 <- "YAL004W"
 YAL004Wexpression <- genedata %>% filter(Gene_ID %in% ID3)
 YAL004Wexpression
 YAL004W <- YAL004Wexpression %>%
  as.data.frame() %>% 
  column_to_rownames("Gene_ID") %>%
  t() %>% as.data.frame() %>% 
  rownames_to_column("sample_id") %>% 
...
#converting the dataframe into numeric matrix

genedata
matrix_data <- data.matrix(genedata)
matrix_data # comparing genedata and matrix_data, the "0" have become "1" upon the matrix transformation

#exploring the distribution of the data
hist(matrix_data)
hist(log2(matrix_data), col = "gray", main = "matrix_data log transformed - Histogram")

#replacing "1" values for N/A 
matrix_data[matrix_data == "1"] <- NA 
head(matrix_data, 50)
nrow(matrix_data)

#re-exploring the distribution of the data
hist(log2(matrix_data), col = "gray", main = "matrix_data log transformed - Histogram") #we can see that the NA values do not affect the distribution anymore

# For our expression data are the number of rows without any NA value, the same as total number of rows?  
dim(matrix_data) == dim(matrix_data[complete.cases(matrix_data),])

#confirming that 0 values have been transformed to NA
YAL004W
